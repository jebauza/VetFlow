...
use App\ZapatoFeroz\Syschronize\Shopify\Etl\Queries\OrdersEtlQuery;
use Zeus\Synchronize\Base\Contracts\Container\OptionsRequestContract;
use App\ZapatoFeroz\Syschronize\Shopify\Etl\Transformers\OrderEtlTransform;

class OrderEtlRepository implements MultiRepo
{
    protected ShopifyConnector $connector;
    protected OrdersEtlQuery $query;
    protected OptionsRequestContract $options;
    protected OrderEtlTransform $transform;
    protected int $limit = 250;

    public function __construct()
    {
        $this->connector = resolve(ShopifyConnector::class);
        $this->query = resolve(OrdersEtlQuery::class);
        $this->options = resolve(OptionsRequestContract::class);
        $this->transform = resolve(OrderEtlTransform::class);
    }
        ...
        $lazyCollection = LazyCollection::make(function () {
            ...
            foreach ($shopifyServers as $shopifyServer) {
                ...
                do {
                    ...
                    if (!empty($response[ShopifyConnector::BODY]->{OrderEtlFields::ORDERS})) {
                        foreach ($response[ShopifyConnector::BODY]->{OrderEtlFields::ORDERS} as $order) {
                            $order->{OrderEtlFields::DOMAIN} = $shopifyServer['domain'];
                            yield $order;
                        }
